...
/**
 * Reads the X-Auth-* headers injected by the API Gateway's JwtAuthenticationFilter
 * and populates Spring's SecurityContext so @PreAuthorize and
 * @AuthenticationPrincipal work in this service.
 *
 * Trust model: The gateway validates JWT; this service trusts the gateway.
 * In production the transaction-service is not directly reachable — only the gateway can call it.
 */
@Slf4j
@Component
